...
/**
 *   	\file       verbruiken_list.php
 *		\ingroup    bpverbruik
 *		\brief      List page for verbruiken
 */
...
require_once __DIR__.'/class/verbruiken.class.php';
// for other modules
...
$arrayfields['total_value'] = array('type'=>'double', 'label'=>'TotalValue', 'checked'=>1, 'enabled'=>1, 'position'=>300, 'csslist'=>'right');
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('bpverbruik', 'verbruiken', 'write');
	$permissiontodelete = $user->hasRight('bpverbruik', 'verbruiken', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("bpverbruik")) {
	accessforbidden('Module bpverbruik not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Verbruiken';
	$objectlabel = 'Verbruiken';
...
$title = $langs->trans("Verbruikens");
//$help_url = "EN:Module_Verbruiken|FR:Module_Verbruiken_FR|ES:Módulo_Verbruiken";
$help_url = '';
$morejs = array();
$morecss = array();
...
$sql .= ", p.label as product_label, p.pmp as product_pmp, (t.qty * p.pmp) as total_value";
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/bpverbruik/verbruiken_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-bpverbruik page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/bpverbruik/verbruiken_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVerbruikenRef";
$modelmail = "verbruiken";
$objecttmp = new Verbruiken($db);
...
// Total value search field (empty for now)
if (!empty($arrayfields['total_value']['checked'])) {
...
// Total value column header
if (!empty($arrayfields['total_value']['checked'])) {
	print '<th class="liste_titre right">'.$langs->trans("TotalValue").'</th>';
	$totalarray['nbfield']++;
}
// Action column
if (!getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields['total_value']['checked'])) {
			...
			print price($obj->total_value, 0, $langs, 1, -1, -1, $conf->currency);
			print '</td>';
			if (!$i) {
				$totalarray['nbfield']++;
			}
			if (!$i) {
				$totalarray['pos'][$totalarray['nbfield']] = 'total_value';
			}
			if (!isset($totalarray['val'])) {
				$totalarray['val'] = array();
			}
			if (!isset($totalarray['val']['total_value'])) {
				$totalarray['val']['total_value'] = 0;
			}
			$totalarray['val']['total_value'] += $obj->total_value;
		}
